...
class NvidiaRepository:
    ...
    def get_status(self) -> Optional[Status]:
        ...
        try:
            ...
            nv.open()
            gpu_count = nv.get_gpu_count()
            gpu_status_list: List[GpuStatus] = []
            for gpu_index in range(gpu_count):
                handle = py3nvml.nvmlDeviceGetHandleByIndex(gpu_index)
                gpu = GPU(gpu_index)
                gpu_utilization = nv.get_gpu_utilization(gpu)
                info = Info(
                    name=nv.get_name(gpu),
                    vbios=nv.get_vbios_version(gpu),
                    driver=nv.get_driver_version(gpu),
                    pcie_generation=nv.get_pcie_generation(gpu),
                    pcie_current_link=nv.get_pcie_current_link_width(gpu),
                    pcie_max_link=nv.get_pcie_max_link_width(gpu),
                    cuda_cores=nv.get_cuda_cores(gpu),
                    uuid=nv.get_gpu_uuid(gpu),
                    memory_total=nv.get_total_dedicated_gpu_memory(gpu),
                    memory_used=nv.get_used_dedicated_gpu_memory(gpu),
                    memory_interface=nv.get_memory_bus_width(gpu),
                    memory_usage=gpu_utilization.get('memory'),
                    gpu_usage=gpu_utilization.get('graphics'),
                    encoder_usage=nv.get_video_encoder_utilization(gpu),
                    decoder_usage=nv.get_video_decoder_utilization(gpu)
                )
                ...
                if mem_transfer_rate_offset_range:
                    overclock = Overclock(
                        ...
                        memory_range=(mem_transfer_rate_offset_range[0] // 2, mem_transfer_rate_offset_range[1] // 2),
                        memory_offset=nv.get_mem_transfer_rate_offset(gpu) // 2,
